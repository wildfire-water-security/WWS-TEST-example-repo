...
#This repository contains large files (`example-large-file.csv` and `example-large-raster.tif`)
#stored on Box (https://oregonstate.app.box.com/folder/336696034807).
...
# section 1: loading large files from Box ------
  ...
  #let's load a dataset directly from Box using boxrdrive
    data <- read.csv(file.path("data/example-large-data.csv"))

  #we can also load spatial data
    dem <- terra::rast(file.path("data/example-large-raster.tif"))
    plot(dem)
